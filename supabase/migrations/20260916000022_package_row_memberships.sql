...
-- ---------------------------------------------------------------------------
-- 1b. Backfill memberships for existing databases.
-- The API now reads package_row_memberships, so an existing installation whose
-- rows predate this table would otherwise lose all published content. Every
-- row that already carries an origin_package_id becomes a member of that
-- package. (Rows with NULL origin are legacy/other-package and are not claimed,
-- consistent with "NULL is not publishable".)
-- ---------------------------------------------------------------------------
