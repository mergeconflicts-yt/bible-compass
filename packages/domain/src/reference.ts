...
function parseVerse(text: string): VerseCoordinate {
  if (hasNonAscii(text)) {
    throw new DomainError('invalid-unicode', `"${text}" contains non-ASCII where ASCII is required.`);
  }
  const m = VERSE_PATTERN.exec(text.trim());
  if (!m) {
    throw new DomainError('format', `"${text}" is not a reference we understand. Try Neh.2.4.`);
  }
  const [, book, ch, vs] = m as unknown as [string, string, string, string];
  if (!BOOKS.has(book)) {
    throw new DomainError('unsupported-book', `"${book}" is not available in this build yet.`);
  }
...
function parseChapter(text: string): VerseCoordinate {
  ...
  if (!m) {
    throw new DomainError('format', `"${text}" is not a reference we understand.`);
  }
...
export function parseQualifiedReference(
  ...
  if (hasNonAscii(input)) {
    throw new DomainError('invalid-unicode', `"${input}" contains non-ASCII where ASCII is required. Use English book codes.`);
  }
  if (!ALLOWED_REFSYS.has(refsys)) {
    throw new DomainError('unsupported-reference-system', `"${refsys}" is not a supported reference system.`);
  }
  if (opts?.editionRefsys && opts.editionRefsys !== refsys) {
    throw new DomainError(
      'mismatched-refsys-edition',
      `Edition refsys "${opts.editionRefsys}" does not match requested refsys "${refsys}".`,
    );
  }
  ...
  return {
    ...
    canonicalKey,
    refsys,
    qualifiedKey: `${refsys}:${canonicalKey}`,
  };
}
