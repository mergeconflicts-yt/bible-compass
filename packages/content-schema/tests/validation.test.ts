...
describe('strict validation — positive and malformed fixtures', () => {
  ...
  it('rejects unknown field', () => {
    ...
    try {
      validateTranslationEdition({ extra: 'x' } as any);
    } catch (e) {
      expect((e as ValidationError).code).toBe('unknown-field');
    }
  });
  ...
  it('rejects duplicate keys', () => {
    expect(() => checkDuplicateKeys([{ key: 'entity:a' }, { key: 'entity:a' }] as any)).toThrow('Duplicate');
  });

  it('rejects dangling reference', () => {
    expect(() =>
      checkDanglingReferences([{ entityKey: 'entity:missing' }], [{ key: 'entity:exists' }] as any),
  ...
  it('rejects invalid state and rights-unknown', () => {
    ...
    expect(() =>
      validateOperationGrant({ componentKey: 'x', operation: 'publication', state: 'unknown' }),
    ).toThrow();
    try {
      validateOperationGrant({ componentKey: 'x', operation: 'publication', state: 'unknown' } as any);
    } catch (e) {
      expect((e as ValidationError).code).toBe('rights-unknown');
    }
  });
