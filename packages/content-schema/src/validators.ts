...
export class ValidationError extends Error {
  ...
  constructor(code: ValidationErrorCode, message: string, issues: z.ZodIssue[]) {
    super(message);
...
function validate<T>(schema: z.ZodType<T>, data: unknown, code: ValidationErrorCode): T {
  const result = schema.safeParse(data);
  if (!result.success) {
    const first = result.error.issues[0];
    // Map zod unknown-key to our code
    const hasUnknown = result.error.issues.some((i) => i.code === 'unrecognized_keys');
    const mappedCode: ValidationErrorCode = hasUnknown ? 'unknown-field' : code;
    throw new ValidationError(mappedCode, first?.message ?? 'Validation failed', result.error.issues);
  }
  return result.data;
}
...
export function validateOperationGrant(data: unknown) {
  ...
  if (!result.success) {
    ...
    const isRightsUnknown = result.error.issues.some((i) => i.message.includes('unknown state must be treated as denied'));
  ...
  if (result.data.state === 'unknown') {
    throw new ValidationError('rights-unknown', 'Publication with unknown rights must fail', []);
  }
  return result.data;
}

// Deterministic duplicate-key check for batch validation
export function checkDuplicateKeys<T extends { key: string }>(records: T[]): void {
  const seen = new Set<string>();
  for (const r of records) {
    if (seen.has(r.key)) {
      throw new ValidationError('duplicate-key', `Duplicate record key: ${r.key}`, []);
    }
    seen.add(r.key);
  }
}
...
export function checkDanglingReferences(
  ...
  for (const a of attestations) {
    if (!entityKeys.has(a.entityKey)) {
      throw new ValidationError('dangling-reference', `Attestation references unknown entity ${a.entityKey}`, []);
    }
  }
}
