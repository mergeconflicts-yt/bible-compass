...
export const canonKeySchema = z.string().regex(/^canon:prot-66$/, 'Invalid canon key');
export const refsysKeySchema = z.enum(['refsys:eng-v22', 'refsys:tel-v1', 'refsys:tam-v1']);
export const workKeySchema = z.string().regex(/^work:[A-Za-z1-9]+:prot-66$/, 'Invalid work key');
export const scopeKeySchema = z.string().regex(/^scope:[a-z0-9-]+:refsys:(eng-v22|tel-v1|tam-v1):[A-Za-z1-9]+\.\d+.*$/, 'Invalid scope key');
export const transWorkSchema = z.string().regex(/^trans:(bsb|tel_irv|tam_irv)$/, 'Invalid translation work');
export const editionKeySchema = z.string().regex(/^edition:(bsb|tel_irv|tam_irv)@[0-9]{8}:sha-[0-9a-f]{8}$/, 'Invalid edition key');
export const sourceKeySchema = z.string().regex(/^source:[a-z0-9:.-]+$/, 'Invalid source key');
export const sourceReleaseKeySchema = z
  .string()
  .regex(/^release:source:[a-z0-9:.-]+@[a-z0-9._-]+:sha-[0-9a-f]{8,64}$/, 'Invalid source release key');
...
export const translationEditionSchema = z
  .object({
    ...
    sourceArtifactSha256: z.string().regex(/^sha256:[0-9a-f]{64}$/, 'Invalid sha256'),
...
export const entitySchema = z
  .object({
    ...
    type: z.enum(['person', 'place', 'collective', 'polity', 'role', 'object', 'structure', 'practice', 'institution', 'theme']),
    identificationStatus: z.enum(['established', 'traditional', 'proposed', 'disputed', 'unknown']),
...
export const claimSchema = z
  .object({
    ...
    object: z.unknown(),
    evidenceStatus: z.enum(['established', 'probable', 'possible', 'disputed', 'unknown']),
    textualBasis: z.enum(['explicit', 'strongly_implied', 'inferred', 'disputed']),
...
export const referenceEntityAttestationSchema = z
  .object({
    ...
    referenceSystem: refsysKeySchema,
    localKey: z.string().regex(/^[A-Za-z1-9]+\.\d+(\.\d+)?(-[A-Za-z1-9]+\.\d+\.\d+)?$/, 'Invalid localKey'),
...
export const editionMentionSchema = z
  .object({
    ...
    entityKey: z.string().regex(/^entity:[a-z0-9-]+$/).optional(),
    contextCardId: z.string().uuid().optional(),
    form: z.enum(['explicit_name', 'alias', 'title', 'pronoun', 'indirect', 'collective', 'unnamed']),
