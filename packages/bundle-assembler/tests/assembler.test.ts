...
describe("Task 19A — Bundle assembler", () => {
  const baseOpts = {
    ...
    gateD2PacketSha: "sha256:a1da9ecef6bd8d61b4433d5733a6587434a391dab023f6c6864271254d91517b",
  ...
  it("input bundle contains only authorized exact source components", () => {
    ...
    expect(bundle.excerpts.every((e) => ["source:stepbible:tipnr", "source:bibledata:structured"].includes(e.sourceKey))).toBe(true);
    expect(bundle.excerpts.every((e) => e.locator.startsWith("TIPNR:") || e.locator.startsWith("BibleData-"))).toBe(true);
  });

  it("registry denial prevents bundle creation", () => {
    expect(() =>
      createBundle({ ...baseOpts, gateD2PacketSha: "invalid" as unknown as string }),
    ).toThrow(/Gate D2/);
  });
  ...
  it("bundle is one scope and one package kind with explicit limits", () => {
    const bundle = createBundle(baseOpts);
    expect(bundle.manifest.target.scope_key).toBe("scope:neh-2:refsys:eng-v22:Neh.2.1-Neh.2.20");
  ...
  it("no scholarly corpus produces explicit context-claim restrictions", () => {
    const bundle = createBundle({ ...baseOpts, scholarlyCorpusApproved: false });
  ...
  it("internet-disabled prompt template", () => {
    ...
    expect(bundle.excerpts.every((e) => !e.text.includes("https://"))).toBe(true);
  });
});
