...
def empty_verses(dataset_id):
    """Bundled verses with no surviving text. Any hit means the extractor
    dropped structured content again — fail the build instead of shipping
    silent gaps (finding 1). Returns osis/chapter/verse labels."""
    ...
    for name in sorted(os.listdir(assets)):
        ...
        for chapter in book["chapters"]:
            for block in chapter["blocks"]:
                if block.get("t") == "v" and not (block.get("text") or "").strip():
                    empty.append(f"{dataset_id} {book['osis']} {chapter['n']}:{block['n']}")
    return empty
...
def main():
    ...
    for dataset_id, asset_dir in TRANSLATIONS:
        ...
        failures.extend(empty_verses(dataset_id))
    if failures:
        for item in failures[:20]:
            print(f"EMPTY VERSE: {item}")
        raise SystemExit(f"refusing output with {len(failures)} empty verses (finding 1)")
    print("verse check: no empty verses in any bundled translation")
