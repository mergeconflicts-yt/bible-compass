...
> **Status note (whole-Bible draft track).** A machine-generated *draft* whole-English corpus (66 books, contract 2.0.0, `synthetic_fixture`) is generated by `tools/curate_canon_queue.py` into `content/curated/` and imported by `tools/import-english-canon.py`. It is unreviewed draft data layered on this same `private_staging` model (canonical identities, edition mentions, locale context); the Nehemiah 2 slice remains the only reviewed/publishable corpus. See `supabase/migrations/20260916000016`–`20260916000019` for the draft-track additions (ownership, publication integrity, external ids, uniqueness/indexes).
