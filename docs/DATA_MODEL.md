...
*Allow/deny tests per `docs/SECURITY.md:55-65` must pass: anon cannot read drafts, User A cannot access User B rows, forged `user_id` insert fails.*
...
* Next: Gate A2 must list *exact* root workspace/package/version changes this section proposes; generic "add needed deps" is invalid.
